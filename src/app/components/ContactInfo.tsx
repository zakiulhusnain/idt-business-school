...
export default function ContactInfo({
  ...
  const handleTabChange = (city: City) => {
    startTransition(() => {
      onCityChange(city)
    })
  }
  ...
  return (
    <section className="w-full bg-gray-50/50 py-16 md:py-24 px-4 sm:px-6 lg:px-8 overflow-hidden">
      <div className="max-w-5xl mx-auto">
        ...
        {/* City Tabs */}
        <div className="flex sm:justify-center items-center gap-2.5 sm:gap-4 mb-8 md:mb-12 overflow-x-auto pb-3 sm:pb-0 px-2 -mx-4 sm:mx-0">
              ...
              <button
                ...
                className={`px-6 sm:px-10 py-3 text-xs sm:text-sm font-extrabold tracking-wide border rounded-xl sm:rounded-2xl transition-all duration-300 flex-shrink-0 cursor-pointer active:scale-[0.97] ${
                  isSelected
                    ...
                    : 'bg-white text-gray-600 border-gray-200 hover:border-[#16509A] hover:text-[#16509A]'
                }`}
              >
                {city}
              </button>
        ...
        <div className={`bg-white p-5 sm:p-8 lg:p-12 rounded-[24px] sm:rounded-[32px] shadow-[0_12px_45px_rgba(0,0,0,0.04)] border border-gray-100 flex flex-col md:flex-row gap-8 lg:gap-12 items-stretch transition-opacity duration-300 ${isPending ? 'opacity-50' : 'opacity-100'}`}>
          ...
          <div className="flex-1 flex flex-col justify-between space-y-6 sm:space-y-8">
            <div className="flex items-start gap-4 group">
              <div className="w-11 h-11 rounded-xl bg-blue-50 flex items-center justify-center shrink-0 group-hover:bg-[#16509A]/10 transition-colors duration-300">
              ...
              <div>
                ...
                <a href={`tel:${locations[activeCity].phone}`} className="text-[#16509A] font-black text-base sm:text-lg hover:underline transition-all">
                  {locations[activeCity].phone}
                </a>
              </div>
            </div>
            ...
            <div className="flex items-start gap-4 group">
              ...
              <div>
                ...
                <a href={`mailto:${locations[activeCity].email}`} className="text-[#16509A] font-black text-base sm:text-lg hover:underline transition-all break-all">
                  {locations[activeCity].email}
                </a>
              </div>
            </div>
          ...
          <div className="flex-1 w-full flex flex-col justify-between gap-6 pt-4 md:pt-0 border-t md:border-t-0 md:border-l border-gray-100 md:pl-8 lg:pl-12">
            ...
            <div className="space-y-3.5 w-full">
              ...
              <div className="flex justify-between items-center border-b border-gray-50 pb-2.5">
                ...
                <span className="font-bold text-sm text-[#16509A] bg-blue-50/60 px-3 py-1 rounded-lg">{locations[activeCity].timings.mon}</span>
              </div>
              <div className="flex justify-between items-center border-b border-gray-50 pb-2.5">
                <span className="font-semibold text-sm text-gray-500">Tuesday–Friday</span>
                <span className="font-bold text-sm text-[#16509A] bg-blue-50/60 px-3 py-1 rounded-lg">{locations[activeCity].timings.tueFri}</span>
              </div>
              <div className="flex justify-between items-center pb-1">
                <span className="font-semibold text-sm text-gray-500">Saturday</span>
                <span className="font-bold text-sm text-[#16509A] bg-blue-50/60 px-3 py-1 rounded-lg">{locations[activeCity].timings.sat}</span>
              </div>
            </div>

            {/* City Image */}
            <div className="w-full aspect-[16/9] relative rounded-2xl overflow-hidden shadow-sm border border-gray-100">
              <Image
                src={locations[activeCity].image}
                alt={activeCity}
                fill
